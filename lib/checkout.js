...
async function cloneAndCheckout(from, to, branch) {
    ...
    if (process.env.TRAVIS) {
        ...
        await afterGit.fetch("origin", `refs/heads/${branch}:refs/remotes/origin/${branch}`);
        await afterGit.remote(["set-branches", "--add", "origin", branch]);
    }
    await afterGit.checkout(branch);
}
...
async function checkout(checkoutDir, beforeBranch, afterBranch) {
    ...
    cloneAndCheckout(checkoutDir, afterDir, afterBranch);
    const beforeDir = path.join(tempDir, "before");
    cloneAndCheckout(checkoutDir, beforeDir, beforeBranch);
