...
async function checkout(checkoutDir, beforeBranch, afterBranch) {
    ...
    if (process.env.TRAVIS) {
        ...
        await git.fetch("origin", `refs/heads/${afterBranch}:refs/remotes/origin/${afterBranch}`);
        await git.remote(["set-branches", "--add", "origin", afterBranch]);
    }
    ...
    if (process.env.TRAVIS) {
        // travis does a minimal branch only & detached head checkout
        await git.fetch("origin", `refs/heads/${beforeBranch}:refs/remotes/origin/${beforeBranch}`);
        await git.remote(["set-branches", "--add", "origin", beforeBranch]);
    }
