...
async function getAfterBranch(gitDir, branch) {
    debug("current branch", currentBranch(gitDir));
    debug("GITHUB_SHA", process.env.GITHUB_SHA);
    return branch || process.env.TRAVIS_BRANCH || process.env.CIRCLE_BRANCH || process.env.GITHUB_SHA || currentBranch(gitDir);
}
