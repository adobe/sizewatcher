...
async function hasRemoteTrackingBranch(dir) {
    console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> hasRemoteTrackingBranch()');
    try {
        const git = Git(dir);
        // git will fail with non-zero exit code if no remote tracking branch is found
...
/*
    if (await hasRemoteTrackingBranch(gitDir)) {
    // if (process.env.CI || await hasRemoteTrackingBranch(gitDir)) {
    // if (process.env.CI) {
