...
module.exports = {
    ...
    compare: async function(before, after) {
        ...
        function printTree(dir) {
            debug(`tree listing of ${dir}`);
            debug(execSync('find . | sed -e "s/[^-][^\\/]*\\//  |/g" -e "s/|\\([^ ]\\)/|-\\1/"', { cwd: dir }).toString());
        }

        printTree(before.dir);
        printTree(after.dir);

        const largestFiles = await getLargestFiles(after.dir);
        const largestCommits = await getLargestCommits(before.dir, before.branch, after.sha);
