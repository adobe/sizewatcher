...
async function runComparator(name, comparator, before, after, deltas) {
    if (await comparator.shouldRun(before.dir, after.dir)) {
        ...
        debug(`skipping '${name}' because it detected it should not run`);
    }
}
    ...
    for (const name of Object.keys(COMPARATORS)) {
        ...
        if (name === "custom") {
            ...
            for (const customConfig of customConfigs) {
                ...
                const customName = customConfig.name || customConfig.path;
                await runComparator(customName, comparator, before, after, deltas);
            }
            ...
            await runComparator(name, comparator, before, after, deltas);
        }
    }
