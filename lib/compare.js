...
function process(deltas) {
    ...
    for (const d of deltas) {
        ...
        if (exceeds(d.afterSize, increase, config.limits.fail)) {
            d.result = "fail";
            summary.fail++;

        } else if (exceeds(d.afterSize, increase, config.limits.warn)) {
            d.result = "warn";
            summary.warn++;

        } else if (exceeds(d.afterSize, increase, config.limits.ok)) {
            d.result = "ok";
            summary.ok++;
