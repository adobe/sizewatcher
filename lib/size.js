...
let getFolderSizePromise;

// lazy load get-folder-size which is an ES module
function loadGetFolderSize() {
    if (!getFolderSizePromise) {
        getFolderSizePromise = import("get-folder-size").then((module) => module.default);
    }
    return getFolderSizePromise;
}
...
async function getFileSize(dir, options) {
    const getFolderSize = await loadGetFolderSize();

    const result = await getFolderSize(dir, options);
    if (result.errors) {
